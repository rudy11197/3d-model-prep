//-----------------------------------------------------------------------------
// Based on the Microsoft sample available from:
// http://create.msdn.com/en-US/education/catalog/sample/collision
//-----------------------------------------------------------------------------
using System;
...
namespace Engine
{
    ...
    public struct BoundingOrientedBox : IEquatable<BoundingOrientedBox>
    {
        ...
        /// <summary>
        /// Create an oriented box from the vertices of its corners.
        /// If the corners do not form a uniform box a best guess is taken!
        /// 
        /// Ends...
        /// 
        ///    0----1  4----5
        ...
        ///    4----7  5----6
        ///    
        /// Each end must form a plane in one axis as must each side.
        /// </summary>
        public BoundingOrientedBox(Vector3[] corners)
        {
            HalfExtent = Vector3.Zero;
            Matrix orient = Matrix.Identity;
            // Right handed coordinate system.
            // +X = Right, +Y = Up, +Z = Backward
            Vector3 extent = Vector3.Subtract(corners[0], corners[1]);
            HalfExtent.X = extent.Length() * 0.5f;
            orient.Right = SafeNormalize(extent);
            extent = Vector3.Subtract(corners[1], corners[2]);
            HalfExtent.Y = extent.Length() * 0.5f;
            orient.Up = SafeNormalize(extent);
            extent = Vector3.Subtract(corners[1], corners[5]);
            HalfExtent.Z = extent.Length() * 0.5f;
            orient.Backward = SafeNormalize(extent);
            Orientation = Quaternion.CreateFromRotationMatrix(orient);
            Vector3 one = Vector3.Divide(Vector3.Add(corners[0], corners[2]), 2);
            Vector3 two = Vector3.Divide(Vector3.Add(corners[5], corners[7]), 2);
            Center = Vector3.Divide(Vector3.Add(one, two), 2);
        }
        ...
        public static Vector3 SafeNormalize(Vector3 vector)
        {
            ...
            return Vector3.Divide(vector, length);
        }
        #endregion
    }
}
